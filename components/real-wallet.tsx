...
type WalletSnapshot = {
  ...
  x3Hold?:{earliestExpiresAt:string|null};
  serverTime?:string;
};
...
export default function RealWallet(){
 ...
 const section=(title:string,items:[string,string,boolean?][])=><section className="wallet-summary-section"><h2>{title}</h2><div className="wallet-summary-grid">{items.map(([label,value,accent])=><article className="smart-glass-card wallet-summary-item" key={label}><small>{label}</small><b className={accent?"text-[#00f77a]":""}>{value}</b>{label==="X3 Hold Wallet"&&data.x3Hold?.earliestExpiresAt&&<X3HoldCountdown serverTime={data.serverTime||data.booster.server_time} expiresAt={data.x3Hold.earliestExpiresAt} onRefresh={load}/>} {label==="Booster Wallet"&&<BoosterCountdown serverTime={data.booster.server_time} nextEntryAt={data.booster.next_entry_at} eligibility={data.booster.eligibility} onRefresh={load} compact/>}</article>)}</div></section>;
...
export function formatX3HoldRemaining(milliseconds:number){
 const seconds=Math.max(0,Math.ceil(milliseconds/1000)),hours=Math.floor(seconds/3600),minutes=Math.floor(seconds%3600/60),remainder=seconds%60;
 return `${String(hours).padStart(2,"0")}:${String(minutes).padStart(2,"0")}:${String(remainder).padStart(2,"0")}`;
}

export function X3HoldCountdown({serverTime,expiresAt,onRefresh}:{serverTime:string;expiresAt:string;onRefresh:()=>Promise<void>}){
 const offset=useRef(Date.now()-new Date(serverTime).getTime()),lastRefresh=useRef(Number.NEGATIVE_INFINITY),[remaining,setRemaining]=useState(()=>new Date(expiresAt).getTime()-(Date.now()-offset.current));
 useEffect(()=>{
  offset.current=Date.now()-new Date(serverTime).getTime();
  const update=()=>{
   const value=new Date(expiresAt).getTime()-(Date.now()-offset.current);setRemaining(value);
   if(value<=0&&Date.now()-lastRefresh.current>=30_000){lastRefresh.current=Date.now();void onRefresh()}
  };
  update();const timer=setInterval(update,1000);return()=>clearInterval(timer);
 },[serverTime,expiresAt,onRefresh]);
 return <span className="x3-hold-countdown">Expires in {formatX3HoldRemaining(remaining)}</span>;
}
