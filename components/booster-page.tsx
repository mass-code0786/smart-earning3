...
export function BoosterPage(){
 ...
 return <div className="grid gap-4">
  ...
  <div className="grid gap-3 md:grid-cols-2">{data.entries.map(entry=><article className="smart-glass-card rounded-[22px] p-5" key={entry.id}><div className="flex justify-between"><b>Booster entry #{entry.cycle_number}</b><span className="text-[10px] text-[#00f77a]">{entry.status}</span></div><div className="mt-4 grid grid-cols-3 gap-2">{[1,2,3].map(slot=>{const position=entry.positions.find(value=>value.slotNumber===slot);return <div className={`min-w-0 rounded-xl border p-3 text-center text-[9px] ${position?"border-[#00f77a]/30 bg-[#00f77a]/5":"border-dashed border-white/10"}`} key={slot}><b className="block">{["A","B","C"][slot-1]}</b><span className="block truncate text-[#8b9d94]">{position?.wallet||"Empty"}</span></div>})}</div></article>)}</div>
