...
export function SmartEarningHeader({home=false}:{home?:boolean}){const router=useRouter();return <header className="dash-header sticky top-0 z-50 flex h-14 items-center justify-between px-3 sm:px-4 lg:ml-60 lg:px-7"><div className="flex min-w-0 items-center gap-1"><button type="button" aria-label={home?"Open menu":"Go back"} onClick={()=>home?router.push('/menu'):history.back()} className="dash-icon">{home?<Menu size={19}/>:<ArrowLeft size={18}/>}</button><SmartLogo/></div><div className="flex items-center gap-1">{home&&<button type="button" aria-label="Light theme" className="dash-icon"><Sun size={17}/></button>}<button type="button" aria-label="Log out" onClick={()=>void logoutAndRedirect()} className="dash-icon"><LogOut size={17}/></button></div></header>}
