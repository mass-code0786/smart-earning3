...
import{ArrowLeft,ChartNoAxesCombined,GitBranch,Home,LogOut,Users,Wallet}from'lucide-react';
...
function SmartLogo(){return <Link href="/dashboard" className="header-logo-crop" aria-label="Smart Earning dashboard"><Image src="/logo.png" alt="Smart Earning" width={1536} height={1024} priority className="header-logo-image object-contain"/></Link>}
...
export function SmartEarningHeader(){async function logout(){try{await fetch('/api/auth/logout',{method:'POST',credentials:'same-origin'})}finally{window.location.assign('/login')}}return <header className="dash-header sticky top-0 z-50 flex h-14 items-center justify-between px-3 sm:px-4 lg:ml-60 lg:px-7"><div className="flex min-w-0 items-center gap-1"><button type="button" aria-label="Go back" onClick={()=>history.back()} className="dash-icon"><ArrowLeft size={18}/></button><SmartLogo/></div><div className="flex items-center gap-1"><button type="button" aria-label="Log out" onClick={logout} className="dash-icon"><LogOut size={17}/></button></div></header>}
...
function SmartEarningSidebar(){const path=usePathname();return <aside className="dash-sidebar fixed inset-y-0 left-0 z-50 hidden w-60 p-5 lg:block"><Link href="/dashboard" aria-label="Smart Earning dashboard"><Image src="/logo.png" alt="Smart Earning" width={1536} height={1024} priority className="h-auto w-[132px] object-contain"/></Link><p className="mt-12 px-3 text-[10px] tracking-[.22em] text-[#8B9D94]">NAVIGATION</p><div className="mt-3 space-y-2">{appNav.map(([href,label,I])=>{const active=path===href||path.startsWith(`${href}/`);return <Link key={label} href={href} className={`flex items-center gap-3 rounded-xl px-3 py-3 text-sm ${active?'bg-[#00F77A]/10 text-[#00F77A]':'text-[#8B9D94] hover:text-[#F5FFF9]'}`}><I size={18}/>{label}</Link>})}</div><div className="smart-glass-card absolute bottom-6 left-5 right-5 rounded-2xl p-4 text-xs text-[#8B9D94]">Network<br/><span className="text-[#F5FFF9]">BNB Testnet · Chain 97</span></div></aside>}

export function SmartEarningPageShell({children}:{children:React.ReactNode}){return <div className="dashboard-theme smart-internal-shell fixed-background-shell min-h-screen bg-[#020705] text-[#F5FFF9]"><FixedMetaverseBackground/><SmartEarningSidebar/><SmartEarningHeader/><main className="relative z-[2] mx-auto max-w-[1480px] px-3 pb-[calc(116px+env(safe-area-inset-bottom))] pt-3 sm:px-4 lg:ml-60 lg:px-7 lg:pb-10">{children}</main><SmartEarningBottomNav/></div>}
