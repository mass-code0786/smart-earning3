  ...
  return <div className="grid gap-4">
    ...
    <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">{data.packages.map(item=><article className="smart-glass-card min-w-0 rounded-[22px] p-5" key={item.packageId}>
      <div className="flex items-center justify-between gap-2"><b className="truncate">Package {item.packageId} · {money(item.priceTokenUnits)}</b>
        <span className={`rounded-full border px-2 py-1 text-[9px] ${item.active?"border-[#00f77a]/30 text-[#00f77a]":"border-white/10 text-[#8b9d94]"}`}>{item.active?"ACTIVE":"INACTIVE"}</span></div>
      ...
      <div className="mt-4 grid gap-1 text-[10px] text-[#8b9d94]">
        <Stat label="Current cycle" value={`#${item.currentCycle}`}/><Stat label="Filled / Empty" value={`${item.filledPositions} / ${item.emptyPositions}`}/>
        <Stat label="Recycle count" value={String(item.recycleCount)}/><Stat label="Magic Level income" value={money(item.magicLevelIncome)} accent/>
        <Stat label="Level 2 income" value={money(item.level2Income)} accent/><Stat label="Total X4 earnings" value={money(item.totalEarnings)} accent/>
      </div></article>)}</div>
...
function MatrixSlot({slot,value,className=""}:{slot:number;value?:Slot;className?:string}){return <div className={`${className} min-w-0 rounded-xl border p-2 text-center text-[9px] ${value?"border-[#00f77a]/30 bg-[#00f77a]/5":"border-dashed border-white/10"}`}>
  <b className="block">L{slot<=2?1:2} · {slot}</b><span className="block truncate text-[#8b9d94]">{value?value.wallet:"Empty"}</span></div>}
