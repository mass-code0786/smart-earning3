...
function encoded(name: "UserRegistered" | "PackagePurchased" | "X4Placed"): IndexerLog {
  const values = name === "UserRegistered"
    ? [address, ZeroAddress, ZeroAddress, 1n, 0, 250_000n, 250_000n]
    : name === "PackagePurchased" ? [address, 1, 1_000_000n, 1_000_000n, 5_000_000n, 1n]
      : [address,ZeroAddress,1,0,0,0n,1n];
...
describe("exact ABI event decoding", () => {
  it("recognizes projected and audit-only deployed events", () => {
    ...
    expect(decodedIndexerEventName(encoded("X4Placed"))).toBe("X4Placed");
