...
describe("Magic Level structure UI", () => {
  it("always shows all 20 cards and opens only the selected level in the centered modal", async () => {
    ...
    expect(screen.getByText(user.memberId)).toBeInTheDocument();
  ...
  it("loads subsequent selected-level pages without replacing existing users", async () => {
    authenticatedWalletFetch
      ...
      .mockResolvedValueOnce(new Response(JSON.stringify({ items: [{ ...user, id: "10000000-0000-0000-0000-000000000002", memberId: "20000000-0000-0000-0000-000000000002" }], nextCursor: null }), { status: 200 }));
    render(<MagicLevelStructure/>);
    ...
    expect(await screen.findByText("20000000-0000-0000-0000-000000000002")).toBeInTheDocument();
    expect(screen.getByText(user.memberId)).toBeInTheDocument();
  });
});
